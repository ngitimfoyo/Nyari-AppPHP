...
class ClassMirrorSpec extends ObjectBehavior {
	...
	function it_throws_an_exception_if_class_is_final($class) {
		...
		$this->shouldThrow ( 'Prophecy\Exception\Doubler\ClassMirrorException' )->duringReflect ( $class, array () );
	}
	...
	function it_throws_an_exception_if_interface_provided_instead_of_class($interface) {
		...
		$this->shouldThrow ( 'Prophecy\Exception\InvalidArgumentException' )->duringReflect ( $interface, array () );
	}
	...
	function it_reflects_all_interfaces_methods($interface1, $interface2, $method1, $method2, $method3) {
		...
		$interface1->getMethods ()->willReturn ( array (
				$method1 
		) );
		$interface2->getMethods ()->willReturn ( array (
				$method2,
				$method3 
		) );
		...
		$classNode = $this->reflect ( null, array (
				$interface1,
				$interface2 
		) );
	...
	function it_throws_an_exception_if_class_provided_in_interfaces_list($class) {
		...
		$this->shouldThrow ( 'InvalidArgumentException' )->duringReflect ( null, array (
				$class 
		) );
	}
