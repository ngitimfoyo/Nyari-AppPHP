...
class ScalarMatcher implements MatcherInterface {
	...
	public function positiveMatch($name, $subject, array $arguments) {
		...
		if (! call_user_func ( $checker, $subject )) {
			throw new FailureException ( sprintf ( '%s expected to return %s, but it did not.', $this->presenter->presentString ( sprintf ( '%s(%s)', $checker, $this->presenter->presentValue ( $subject ) ) ), $this->presenter->presentValue ( true ) ) );
		}
	}
