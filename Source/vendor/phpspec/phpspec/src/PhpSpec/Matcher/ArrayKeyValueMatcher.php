...
class ArrayKeyValueMatcher extends BasicMatcher {
	...
	public function supports($name, $subject, array $arguments) {
		return (is_array ( $subject ) || $subject instanceof \ArrayAccess) && 'haveKeyWithValue' === $name && 2 == count ( $arguments );
	}
	...
	protected function getFailureException($name, $subject, array $arguments) {
		...
		if (! $this->offsetExists ( $key, $subject )) {
			return new FailureException ( sprintf ( 'Expected %s to have key %s, but it didn\'t.', $this->presenter->presentValue ( $subject ), $this->presenter->presentString ( $key ) ) );
		}
		
		return new FailureException ( sprintf ( 'Expected %s to have value %s for %s key, but found %s.', $this->presenter->presentValue ( $subject ), $this->presenter->presentValue ( $arguments [1] ), $this->presenter->presentString ( $key ), $this->presenter->presentValue ( $subject [$key] ) ) );
	}
