...
class StringPresenter implements PresenterInterface {
	...
	public function presentValue($value) {
		...
		if ($value instanceof Exception) {
			return $this->presentString ( sprintf ( '[exc:%s("%s")]', get_class ( $value ), $value->getMessage () ) );
		}
	...
	protected function presentExceptionStackTrace(Exception $exception) {
		...
		$text .= $this->presentExceptionTraceFunction ( 'throw new ' . get_class ( $exception ), array (
		...
		foreach ( $exception->getTrace () as $call ) {
			...
			if (isset ( $call ['class'] )) {
				$text .= $this->presentExceptionTraceMethod ( $call ['class'], $call ['type'], $call ['function'], isset ( $call ['args'] ) ? $call ['args'] : array () );
			} elseif (isset ( $call ['function'] )) {
				$text .= $this->presentExceptionTraceFunction ( $call ['function'], isset ( $call ['args'] ) ? $call ['args'] : array () );
			}
		}
