...
class ArrayContainMatcherSpec extends ObjectBehavior {
	...
	function it_responds_to_contain() {
		$this->supports ( 'contain', array (), array (
				'' 
		) )->shouldReturn ( true );
	}
	function it_matches_array_with_specified_value() {
		$this->shouldNotThrow ()->duringPositiveMatch ( 'contain', array (
				'abc' 
		), array (
				'abc' 
		) );
	...
	function it_does_not_match_array_without_specified_value() {
		$this->shouldThrow ()->duringPositiveMatch ( 'contain', array (
				1,
				2,
				3 
		), array (
		...
		$this->shouldThrow ( 'PhpSpec\Exception\Example\FailureException' )->duringPositiveMatch ( 'contain', array (
				1,
				2,
				3 
		), array (
	...
	function it_matches_array_without_specified_value() {
		$this->shouldNotThrow ()->duringNegativeMatch ( 'contain', array (
				1,
				2,
				3 
		), array (
