...
class ObjectStateMatcherSpec extends ObjectBehavior {
	...
	function it_throws_exception_if_checker_method_not_found() {
		...
		$this->shouldThrow ( 'PhpSpec\Exception\Fracture\MethodNotFoundException' )->duringPositiveMatch ( 'beSimple', $subject, array () );
	}
	...
	function it_does_not_match_if_state_checker_returns_false() {
		...
		$this->shouldThrow ( 'PhpSpec\Exception\Example\FailureException' )->duringPositiveMatch ( 'beFinal', $subject, array () );
	}
	function it_infers_matcher_alias_name_from_methods_prefixed_with_has() {
		$subject = new \ReflectionClass ( $this );
		
		$this->supports ( 'haveProperty', $subject, array (
				'something' 
		) )->shouldReturn ( true );
	}
	function it_throws_exception_if_has_checker_method_not_found() {
		...
		$this->shouldThrow ( 'PhpSpec\Exception\Fracture\MethodNotFoundException' )->duringPositiveMatch ( 'haveAnything', $subject, array (
				'str' 
		) );
	}
	function it_matches_if_has_checker_returns_true() {
		...
		$this->shouldNotThrow ()->duringPositiveMatch ( 'haveMethod', $subject, array (
	...
	function it_does_not_match_if_has_state_checker_returns_false() {
		...
		$this->shouldThrow ( 'PhpSpec\Exception\Example\FailureException' )->duringPositiveMatch ( 'haveProperty', $subject, array (
				'other' 
		) );
	}
