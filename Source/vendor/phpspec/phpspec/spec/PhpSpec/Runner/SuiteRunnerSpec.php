...
class SuiteRunnerSpec extends ObjectBehavior {
	function let(EventDispatcher $dispatcher, SpecificationRunner $specRunner, Suite $suite, SpecificationNode $spec1, SpecificationNode $spec2) {
		...
		$suite->getSpecifications ()->willReturn ( array (
				$spec1,
				$spec2 
		) );
	}
	...
	function it_dispatches_events_before_and_after_the_suite($suite, $dispatcher) {
		...
		$dispatcher->dispatch ( 'beforeSuite', Argument::type ( 'PhpSpec\Event\SuiteEvent' ) )->shouldHaveBeenCalled ();
		
		$dispatcher->dispatch ( 'afterSuite', Argument::type ( 'PhpSpec\Event\SuiteEvent' ) )->shouldHaveBeenCalled ();
	}
