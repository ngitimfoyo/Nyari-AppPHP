...
class MatcherManagerSpec extends ObjectBehavior {
	...
	function it_searches_in_registered_matchers(MatcherInterface $matcher) {
		...
		$matcher->supports ( 'startWith', 'hello, world', array (
				'hello' 
		) )->willReturn ( true );
		
		$this->add ( $matcher );
		$this->find ( 'startWith', 'hello, world', array (
				'hello' 
	...
	function it_searches_matchers_by_their_priority(MatcherInterface $matcher1, MatcherInterface $matcher2) {
		...
		$matcher1->supports ( 'startWith', 'hello, world', array (
				'hello' 
		) )->willReturn ( true );
		$matcher2->getPriority ()->willReturn ( 5 );
		$matcher2->supports ( 'startWith', 'hello, world', array (
				'hello' 
		) )->willReturn ( true );
	...
	function it_throws_MatcherNotFoundException_if_matcher_not_found() {
		$this->shouldThrow ( 'PhpSpec\Exception\Wrapper\MatcherNotFoundException' )->duringFind ( 'startWith', 'hello, world', array (
				'hello' 
		) );
	}
}
