...
class ResourceManagerSpec extends ObjectBehavior {
	...
	function it_locates_resources_using_all_registered_locators($locator1, $locator2, ResourceInterface $resource1, ResourceInterface $resource2, ResourceInterface $resource3) {
		...
		$locator1->findResources ( 's:query' )->willReturn ( array (
				$resource3,
				$resource2 
		) );
		$locator2->supportsQuery ( 's:query' )->willReturn ( true );
		$locator2->findResources ( 's:query' )->willReturn ( array (
				$resource1 
		) );
	...
	function it_locates_all_locators_resources_if_query_string_is_empty($locator1, $locator2, ResourceInterface $resource1, ResourceInterface $resource2, ResourceInterface $resource3) {
		...
		$locator1->getAllResources ()->willReturn ( array (
				$resource3,
				$resource2 
		) );
		$locator2->getAllResources ()->willReturn ( array (
				$resource1 
		) );
		...
		$this->locateResources ( '' )->shouldReturn ( array (
				$resource1,
				$resource3,
				$resource2 
		) );
	...
	function it_creates_resource_from_classname_using_locator_with_highest_priority($locator1, $locator2, ResourceInterface $resource1, ResourceInterface $resource2) {
		$this->registerLocator ( $locator1 );
		$this->registerLocator ( $locator2 );
	...
	function it_does_not_allow_two_resources_for_the_same_spec($locator1, $locator2, ResourceInterface $resource1, ResourceInterface $resource2) {
		$this->registerLocator ( $locator1 );
		$this->registerLocator ( $locator2 );
