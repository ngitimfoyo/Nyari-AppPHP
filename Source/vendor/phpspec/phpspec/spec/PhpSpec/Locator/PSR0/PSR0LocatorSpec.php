...
class PSR0LocatorSpec extends ObjectBehavior {
	...
	function it_generates_proper_fullSpecPath_even_from_empty_src_namespace() {
		...
		$this->getFullSpecPath ()->shouldReturn ( __DIR__ . DIRECTORY_SEPARATOR . 'spec' . DIRECTORY_SEPARATOR );
	}
	...
	function it_finds_all_resources_from_tracked_specPath(Filesystem $fs, SplFileInfo $file) {
		...
		$fs->findSpecFilesIn ( $path )->willReturn ( array (
				$file 
		) );
	...
	function it_supports_file_queries_in_srcPath() {
		...
		$this->supportsQuery ( realpath ( $this->srcPath . '/PhpSpec/ServiceContainer.php' ) )->shouldReturn ( true );
	}
	...
	function it_supports_file_queries_in_specPath() {
		...
		$this->supportsQuery ( realpath ( $this->specPath . '/spec/PhpSpec/ServiceContainerSpec.php' ) )->shouldReturn ( true );
	}
	...
	function it_finds_spec_resources_via_fullSrcPath(Filesystem $fs, SplFileInfo $file) {
		...
		$fs->findSpecFilesIn ( $this->specPath . $this->convert_to_path ( '/spec/PhpSpec/Console/' ) )->willReturn ( array (
				$file 
		) );
	...
	function it_finds_spec_resources_via_specPath(Filesystem $fs, SplFileInfo $file) {
		...
		$fs->findSpecFilesIn ( $this->specPath . $this->convert_to_path ( '/spec/PhpSpec/Runner/' ) )->willReturn ( array (
				$file 
		) );
	...
	function it_throws_an_exception_on_non_PSR0_resource() {
		...
		$exception = new \InvalidArgumentException ( 'String "Non-PSR0/Namespace" is not a valid class name.' . PHP_EOL . 'Please see reference document: https://github.com/php-fig/fig-standards/blob/master/accepted/PSR-0.md' );
	...
	function it_throws_an_exception_on_PSR4_prefix_not_matching_namespace() {
		...
		$this->shouldThrow ( $exception )->during ( '__construct', array (
				'p\pf\N\S',
				'spec',
				$this->srcPath,
				$this->specPath,
