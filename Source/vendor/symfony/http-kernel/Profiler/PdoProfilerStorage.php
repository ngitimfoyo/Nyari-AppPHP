...
abstract class PdoProfilerStorage implements ProfilerStorageInterface {
	 ...
	 * @param int $lifetime
	 *        	The lifetime to use for the purge
	 */
	 ...
	 *        	The URL
	 * @param string $start
	 *        	The start period to search from
	 * @param string $end
	 *        	The end period to search to
	 * @param string $limit
	...
	protected function cleanup() {
		$db = $this->initDb ();
		$this->exec ( $db, 'DELETE FROM sf_profiler_data WHERE created_at < :time', array (
	...
	protected function readChildren($token, $parent) {
		$db = $this->initDb ();
		$data = $this->fetch ( $db, 'SELECT token, data, ip, method, url, time FROM sf_profiler_data WHERE parent = :token', array (
				':token' => $token 
		) );
		$this->close ( $db );
	...
	protected function has($token) {
		$db = $this->initDb ();
		$tokenExists = $this->fetch ( $db, 'SELECT 1 FROM sf_profiler_data WHERE token = :token LIMIT 1', array (
				':token' => $token 
		) );
		$this->close ( $db );
