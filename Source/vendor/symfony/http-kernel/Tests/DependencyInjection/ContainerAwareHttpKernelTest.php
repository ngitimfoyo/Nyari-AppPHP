...
class ContainerAwareHttpKernelTest extends \PHPUnit_Framework_TestCase {
	...
	public function testHandle($type) {
		...
		$this->expectsEnterScopeOnce ( $container )->expectsLeaveScopeOnce ( $container )->expectsSetRequestWithAt ( $container, $request, 3 )->expectsSetRequestWithAt ( $container, null, 4 );
	...
	public function testVerifyRequestStackPushPopDuringHandle($type) {
		...
		$stack = $this->getMock ( 'Symfony\Component\HttpFoundation\RequestStack', array (
				'push',
				'pop' 
		) );
	...
	private function expectsSetRequestWithAt($container, $with, $at) {
		$container->expects ( $this->at ( $at ) )->method ( 'set' )->with ( $this->equalTo ( 'request' ), $this->equalTo ( $with ), $this->equalTo ( 'request' ) );
