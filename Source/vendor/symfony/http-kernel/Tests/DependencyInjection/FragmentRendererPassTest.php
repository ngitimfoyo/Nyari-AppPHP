...
class FragmentRendererPassTest extends \PHPUnit_Framework_TestCase {
	...
	public function testLegacyFragmentRedererWithoutAlias() {
		...
		$renderer->expects ( $this->once () )->method ( 'addMethodCall' )->with ( 'addRenderer', array (
		...
		$definition->expects ( $this->once () )->method ( 'isPublic' )->will ( $this->returnValue ( true ) );
		...
		$builder->expects ( $this->atLeastOnce () )->method ( 'getDefinition' )->will ( $this->onConsecutiveCalls ( $renderer, $definition ) );
	...
	public function testValidContentRenderer() {
		...
		$renderer = $this->getMock ( 'Symfony\Component\DependencyInjection\Definition' );
		$renderer->expects ( $this->once () )->method ( 'addMethodCall' )->with ( 'addRendererService', array (
				'foo',
				'my_content_renderer' 
		) );
