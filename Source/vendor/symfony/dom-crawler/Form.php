...
class Form extends Link implements \ArrayAccess {
	 ...
	 *        	The URI of the <base> used for relative links, but not for empty action
	 *        	
	...
	protected function setNode(\DOMElement $node) {
		$this->button = $node;
		if ('button' === $node->nodeName || ('input' === $node->nodeName && in_array ( strtolower ( $node->getAttribute ( 'type' ) ), array (
				'submit',
				'button',
				'image' 
		) ))) {
	...
	private function addField(\DOMElement $node) {
		...
		} elseif ('input' == $nodeName && ! in_array ( strtolower ( $node->getAttribute ( 'type' ) ), array (
				'submit',
				'button',
				'image' 
		) )) {
