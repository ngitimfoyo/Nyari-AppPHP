...
class ChoiceFormFieldTest extends FormFieldTestCase {
	...
	public function testIsMultiple() {
		...
		$node = $this->createNode ( 'select', '', array (
				'multiple' => 'multiple' 
		) );
		$field = new ChoiceFormField ( $node );
	...
	public function testMultipleSelects() {
		...
		$this->assertEquals ( array (
				'foo' 
		), $field->getValue (), '->setValue() returns an array of options if multiple is true' );
		...
		$this->assertEquals ( array (
				'foo',
				'bar' 
		), $field->getValue (), '->getValue() returns the selected options' );
	...
	public function testOptionWithNoValue() {
		$node = $this->createSelectNodeWithEmptyOption ( array (
				'foo' => false,
				'bar' => false 
		) );
		$field = new ChoiceFormField ( $node );
	...
	public function testDisableValidation() {
		...
		$this->assertEquals ( array (
				'foobar' 
		), $field->getValue (), '->disableValidation() allows to set a value which is not in the selected options.' );
	}
