...
class LegacyApacheMatcherDumperTest extends \PHPUnit_Framework_TestCase {
	...
	public function testEscapeScriptName() {
		...
		$this->assertStringEqualsFile ( self::$fixturesPath . '/dumper/url_matcher2.apache', $dumper->dump ( array (
	...
	private function getRouteCollection() {
		...
		$collection->add ( 'foobar', new Route ( '/foo/{bar}', array (
				'bar' => 'toto' 
		) ) );
		// method requirement
		...
		$collection->add ( 'baragain', new Route ( '/baragain/{foo}', array (), array (), array (), '', array (), array (
				'get',
				'post' 
		) ) );
		// simple
		...
		$collection->add ( 'baz5unsafe', new Route ( '/testunsafe/{foo}/', array (), array (), array (), '', array (), array (
				'post' 
		) ) );
		// complex
		$collection->add ( 'baz6', new Route ( '/test/baz', array (
				'foo' => 'bar baz' 
		) ) );
		// space in path
		...
		$collection->add ( 'baz9', new Route ( '/test/{baz}', array (), array (
				'baz' => 'te\\\\ st' 
		) ) );
