...
class FinderTest extends Iterator\RealIteratorTestCase {
	...
	public function testIn() {
		$finder = $this->buildFinder ();
		$iterator = $finder->files ()->name ( '*.php' )->depth ( '< 1' )->in ( array (
				self::$tmpDir,
				__DIR__ 
		) )->getIterator ();
	...
	public function testContains($matchPatterns, $noMatchPatterns, $expected) {
		$finder = $this->buildFinder ();
		$finder->in ( __DIR__ . DIRECTORY_SEPARATOR . 'Fixtures' )->name ( '*.txt' )->sortByName ()->contains ( $matchPatterns )->notContains ( $noMatchPatterns );
		
		$this->assertIterator ( $this->toAbsoluteFixtures ( $expected ), $finder );
	}
	public function testContainsOnDirectory() {
		$finder = $this->buildFinder ();
		$finder->in ( __DIR__ )->directories ()->name ( 'Fixtures' )->contains ( 'abc' );
		$this->assertIterator ( array (), $finder );
	}
	public function testNotContainsOnDirectory() {
		$finder = $this->buildFinder ();
		$finder->in ( __DIR__ )->directories ()->name ( 'Fixtures' )->notContains ( 'abc' );
		$this->assertIterator ( array (), $finder );
	}
	...
	public function testAdaptersOrdering() {
		$finder = Finder::create ()->removeAdapters ()->addAdapter ( new FakeAdapter\NamedAdapter ( 'a' ), 0 )->addAdapter ( new FakeAdapter\NamedAdapter ( 'b' ), - 50 )->addAdapter ( new FakeAdapter\NamedAdapter ( 'c' ), 50 )->addAdapter ( new FakeAdapter\NamedAdapter ( 'd' ), - 25 )->addAdapter ( new FakeAdapter\NamedAdapter ( 'e' ), 25 );
	...
	public function testAdaptersChaining() {
		...
		$filenames = $this->toAbsolute ( array (
				'foo',
				'foo/bar.tmp',
				'test.php',
				'test.py',
		...
		$finder = Finder::create ()->removeAdapters ()->addAdapter ( new FakeAdapter\UnsupportedAdapter (), 3 )->addAdapter ( new FakeAdapter\FailingAdapter (), 2 )->addAdapter ( new FakeAdapter\DummyAdapter ( $iterator ), 1 );
	...
	public function testPath($matchPatterns, $noMatchPatterns, array $expected) {
		$finder = $this->buildFinder ();
		$finder->in ( __DIR__ . DIRECTORY_SEPARATOR . 'Fixtures' )->path ( $matchPatterns )->notPath ( $noMatchPatterns );
	...
	private function buildFinder() {
		return Finder::create ()->removeAdapters ()->addAdapter ( $this->getAdapter () );
	}
}
