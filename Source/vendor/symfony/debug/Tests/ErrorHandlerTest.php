...
class ErrorHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testHandleError() {
		try {
			...
			$logger->expects ( $this->once () )->method ( 'log' )->will ( $this->returnCallback ( $warnArgCheck ) );
	...
	public function testHandleException() {
		try {
			...
			$logger->expects ( $this->exactly ( 2 ) )->method ( 'log' )->will ( $this->returnCallback ( $logArgCheck ) );
	...
	public function testErrorStacking() {
		try {
			...
			$logger->expects ( $this->exactly ( 2 ) )->method ( 'log' )->withConsecutive ( array (
					$this->equalTo ( LogLevel::WARNING ),
					$this->equalTo ( 'Dummy log' ) 
			), array (
					$this->equalTo ( LogLevel::DEBUG ),
					$this->equalTo ( 'Silenced warning' ) 
			) );
	...
	public function testHandleFatalErrorOnHHVM() {
		try {
			...
			$logger = $this->getMock ( 'Psr\Log\LoggerInterface' );
			$logger->expects ( $this->once () )->method ( 'log' )->with ( $this->equalTo ( LogLevel::CRITICAL ), $this->equalTo ( 'Fatal Error: foo' ), $this->equalTo ( array (
					'type' => 1,
					'file' => 'bar',
					'line' => 123,
					'level' => - 1,
