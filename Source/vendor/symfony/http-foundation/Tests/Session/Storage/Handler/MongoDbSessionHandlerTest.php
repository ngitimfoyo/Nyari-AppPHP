...
class MongoDbSessionHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	protected function setUp() {
		...
		$this->mongo = $this->getMockBuilder ( $mongoClass )->getMock ();
	...
	public function testRead() {
		...
		$this->mongo->expects ( $this->once () )->method ( 'selectCollection' )->with ( $this->options ['database'], $this->options ['collection'] )->will ( $this->returnValue ( $collection ) );
		...
		$collection->expects ( $this->once () )->method ( 'findOne' )->will ( $this->returnCallback ( function ($criteria) use($that, $testTimeout) {
	...
	public function testWrite() {
		...
		$collection->expects ( $this->once () )->method ( 'update' )->will ( $this->returnCallback ( function ($criteria, $updateData, $options) use($that, &$data) {
	...
	public function testReplaceSessionData() {
		...
		$collection->expects ( $this->exactly ( 2 ) )->method ( 'update' )->will ( $this->returnCallback ( function ($criteria, $updateData, $options) use(&$data) {
			$data = $updateData;
		} ) );
	...
	public function testDestroy() {
		...
		$collection->expects ( $this->once () )->method ( 'remove' )->with ( array (
	...
	public function testGc() {
		...
		$collection->expects ( $this->once () )->method ( 'remove' )->will ( $this->returnCallback ( function ($criteria) use($that) {
