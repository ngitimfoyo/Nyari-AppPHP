...
class PdoSessionHandlerTest extends \PHPUnit_Framework_TestCase {
	...
	public function testReadConvertsStreamToString() {
		...
		$pdo->prepareResult->expects ( $this->once () )->method ( 'fetchAll' )->will ( $this->returnValue ( array (
				array (
						$stream,
						42,
						time () 
	...
	public function testReadLockedConvertsStreamToString() {
		...
		$selectStmt->expects ( $this->atLeast ( 2 ) )->method ( 'fetchAll' )->will ( $this->returnCallback ( function () use(&$exception, $stream) {
		...
		$insertStmt->expects ( $this->once () )->method ( 'execute' )->will ( $this->returnCallback ( function () use(&$exception) {
