...
class ProcessFailedExceptionTest extends \PHPUnit_Framework_TestCase {
	...
	public function testProcessFailedExceptionThrowsException() {
		$process = $this->getMock ( 'Symfony\Component\Process\Process', array (
				'isSuccessful' 
		), array (
				'php' 
		) );
		...
		$this->setExpectedException ( '\InvalidArgumentException', 'Expected a failed process, but the given process was successful.' );
	...
	public function testDisabledOutputInFailedExceptionDoesNotPopulateOutput() {
		...
		$process->expects ( $this->once () )->method ( 'isOutputDisabled' )->will ( $this->returnValue ( true ) );
