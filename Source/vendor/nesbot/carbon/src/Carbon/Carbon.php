...
class Carbon extends DateTime {
	...
	public function diffForHumans(Carbon $other = null, $absolute = false) {
		...
		$time = static::translator ()->transChoice ( $unit, $count, array (
				':count' => $count 
		) );
		...
		if ($tryKeyExists !== static::translator ()->transChoice ( $tryKeyExists, $count )) {
			$time = static::translator ()->transChoice ( $tryKeyExists, $count, array (
					':count' => $count 
			) );
		}
	 ...
	 * in the current month.
	 * If the calculated occurence is outside the scope
